...
import { putCavsafeObjectStream, deleteCavsafeObject } from "@/lib/cavsafe/r2.server";
import { CavSafeError, createFileMetadata, createFolder } from "@/lib/cavsafe/storage.server";
import { softDeleteFile, softDeleteFolder } from "@/lib/cavcloud/storage.server";
...
function cavsafeObjectKey(accountId: string, fileId: string, fileName: string): string {
  return `safe/${accountId}/${fileId}/${safeFilenameForKey(fileName)}`;
}

async function ensureSafeRoot(accountId: string): Promise<{ id: string; path: string }> {
  const existing = await prisma.cavSafeFolder.findFirst({
  ...
  const settled = await prisma.cavSafeFolder.findFirst({
    where: { accountId, path: "/", deletedAt: null },
    select: { id: true, path: true },
  });
  if (settled) return settled;

  const fallback = await prisma.cavSafeFolder.findFirst({
    where: { accountId, path: "/" },
    select: { id: true, path: true, name: true, parentId: true, deletedAt: true },
  });
  if (!fallback) throw new CavSafeError("ROOT_FOLDER_INIT_FAILED", 500, "Failed to initialize CavSafe root folder.");

  if (fallback.deletedAt || fallback.parentId !== null || fallback.name !== "root") {
    return prisma.cavSafeFolder.update({
...
async function ensureSafeFolderPath(args: {
  accountId: string;
  operatorUserId: string;
  path: string;
}): Promise<{ id: string; path: string }> {
  const target = normalizePath(args.path);
  let current = await ensureSafeRoot(args.accountId);
  ...
  for (const partRaw of parts) {
    ...
    const hit = await prisma.cavSafeFolder.findFirst({
...
async function pathConflictExists(accountId: string, path: string): Promise<boolean> {
...
async function uniqueSafeName(args: {
  accountId: string;
  parentPath: string;
  desired: string;
}): Promise<string> {
  ...
  for (let idx = 0; idx < 2048; idx += 1) {
    ...
    if (!(await pathConflictExists(args.accountId, path))) return candidate;
  }
  throw new CavSafeError("PATH_CONFLICT", 409, "Unable to allocate destination path in CavSafe.");
}

async function copyCloudObjectToSafe(args: {
  sourceKey: string;
...
async function cleanupBestEffort(args: {
  ...
  try {
    if (args.safeFileIds.length) {
      await prisma.cavSafeFile.deleteMany({ where: { id: { in: args.safeFileIds } } });
    }
  } catch {
    // best effort
  }
  try {
    if (args.safeFolderIds.length) {
      await prisma.cavSafeFolder.deleteMany({ where: { id: { in: args.safeFolderIds } } });
    }
  } catch {
    // best effort
  }
  await Promise.all(
    args.safeObjectKeys.map(async (key) => {
      try {
...
async function moveFileToCavSafe(args: {
  accountId: string;
  operatorUserId: string;
  fileId: string;
}): Promise<MoveResult> {
  ...
  const destinationParent = await ensureSafeFolderPath({
    accountId: args.accountId,
    operatorUserId: args.operatorUserId,
    path: parentPath(source.path),
  });

  const destinationName = await uniqueSafeName({
    accountId: args.accountId,
  ...
  const destinationKey = cavsafeObjectKey(args.accountId, destinationFileId, destinationName);
  const safeFileIds: string[] = [];
  const safeObjectKeys: string[] = [];

  try {
    await copyCloudObjectToSafe({
      sourceKey: source.r2Key,
      destinationKey,
      contentType: source.mimeType,
      contentLength: Number(source.bytes),
    });
    safeObjectKeys.push(destinationKey);
...
async function moveFolderTreeToCavSafe(args: {
  accountId: string;
  operatorUserId: string;
  folderId: string;
}): Promise<MoveResult> {
  const sourceRoot = await prisma.cavCloudFolder.findFirst({
    where: {
      id: args.folderId,
      accountId: args.accountId,
      deletedAt: null,
  ...
  const rootName = await uniqueSafeName({
    accountId: args.accountId,
    parentPath: destinationParent.path,
    desired: sourceRoot.name,
  });

  const queue: Array<{ sourceFolderId: string; safeFolderId: string }> = [];
  ...
  try {
    const rootCreated = await createFolder({
      accountId: args.accountId,
      operatorUserId: args.operatorUserId,
      parentId: destinationParent.id,
      name: rootName,
    });
    queue.push({ sourceFolderId: sourceRoot.id, safeFolderId: rootCreated.id });
    ...
    while (queue.length > 0) {
      ...
      for (const row of childFolders) {
        const created = await createFolder({
          accountId: args.accountId,
          operatorUserId: args.operatorUserId,
          parentId: next.safeFolderId,
          name: row.name,
        });
        queue.push({ sourceFolderId: row.id, safeFolderId: created.id });
      ...
      for (const row of childFiles) {
        ...
        const nextObjectKey = cavsafeObjectKey(args.accountId, nextFileId, row.name);
        await copyCloudObjectToSafe({
          sourceKey: row.r2Key,
    ...
    await cleanupBestEffort({
      safeFileIds: createdFileIds,
      safeFolderIds: [...createdFolderIds].reverse(),
      safeObjectKeys: copiedObjectKeys,
    });
    throw err;
  }
}

export async function moveFromCavCloudToCavSafe(args: {
  accountId: string;
  operatorUserId: string;
  kind: MoveKind;
  id: string;
  ...
  return moveFolderTreeToCavSafe({
    accountId,
    operatorUserId,
    folderId: id,
  });
