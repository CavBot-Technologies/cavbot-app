...
export function cavsafeR2Configured(): boolean {
  return loadR2Config() !== null;
}

export function cavsafeR2PrefixObjectKey(objectKey: string): string {
...
export async function putCavsafeObject(options: {
  objectKey: string;
  body: Buffer;
  contentType: string;
...
export async function putCavsafeObjectStream(options: {
  objectKey: string;
  body: StreamBody;
  contentType: string;
  contentLength?: number;
...
export async function deleteCavsafeObject(objectKey: string): Promise<void> {
  const cfg = requireR2Config();
...
export async function headCavsafeObject(objectKey: string): Promise<{ bytes: number; etag: string | null } | null> {
  const cfg = requireR2Config();
  const key = r2KeyForObjectKey(objectKey);
  const client = getS3Client(cfg);
...
export async function getCavsafeObjectStream(options: {
  objectKey: string;
  range?: string;
}): Promise<{
  status: number;
...
export async function createCavsafeMultipartUpload(options: {
  objectKey: string;
  contentType: string;
...
export async function uploadCavsafeMultipartPart(options: {
  objectKey: string;
  uploadId: string;
  partNumber: number;
...
export async function completeCavsafeMultipartUpload(options: {
  objectKey: string;
  uploadId: string;
...
export async function abortCavsafeMultipartUpload(options: { objectKey: string; uploadId: string }): Promise<void> {
  const cfg = requireR2Config();
  const key = r2KeyForObjectKey(options.objectKey);
  const uploadId = String(options.uploadId || "").trim();
  if (!uploadId) return;
