...
function constantTimeEqualHeader(a, b) {
...
function trustedProxyClientIp(req, env) {
  const expected = String(env?.CAVBOT_ADMIN_TOKEN || "").trim();
  if (!expected) return "";
  ...
  if (!token || !constantTimeEqualHeader(token, expected)) return "";

  return firstForwardedIp(
    req.headers.get("X-Cavbot-Forwarded-Client-IP") ||
      req.headers.get("x-cavbot-forwarded-client-ip")
  );
}
...
async function enforceRateLimit(req, env, projectId, origin, opts) {
  ...
  const ip = trustedProxyClientIp(req, env) || getIp(req);
...
const worker = {
  async fetch(request, env, ctx) {
    ...
    const adminKeysRouteMatch = path.match(/^\/v1\/admin\/projects\/(\d+)\/keys$/);
    const isAdminKeysPost = Boolean(adminKeysRouteMatch) && isAdmin && request.method === "POST";
    ...
    const BASE_ALLOWED_HEADERS = [
      ...
      "X-Admin-Token",
      "X-Cavbot-Project-Id",
      "X-Cavbot-Verified-Site-Id",
      "X-Cavbot-Forwarded-Client-IP",
    ...
    function normalizeEventName(rawName, rawType) {
      const name = safeString(rawName || rawType || "", 120)
        .trim()
        .toLowerCase()
        .replace(/[.\s-]+/g, "_");
      const type = safeString(rawType || "", 120)
        .trim()
        .toLowerCase()
        .replace(/[.\s-]+/g, "_");
      const key = name || type;
      if (!key) return "";
      if (["page_view", "pageview", "cavbot_pageview", "cavbot_page_view"].includes(key)) return "cavbot_page_view";
      if (["route_change", "routechange", "spa_navigation", "navigation", "cavbot_route_change"].includes(key)) {
        return "cavbot_route_change";
      }
      if (["404", "not_found", "page_404", "cavbot_404", "cavbot_not_found"].includes(key)) return "cavbot_404";
      if (["js_error", "javascript_error", "window_error", "cavbot_js_error"].includes(key)) return "cavbot_js_error";
      if (["api_error", "fetch_error", "http_error", "network_error", "cavbot_api_error"].includes(key)) return "cavbot_api_error";
      if (["error", "cavbot_error"].includes(key) && type.includes("api")) return "cavbot_api_error";
      if (["error", "cavbot_error"].includes(key)) return "cavbot_js_error";
      return safeString(key, 120);
    }

    function normalizeRoutePath(input, pageUrl) {
      const raw = safeString(input || "", 500).trim();
      let value = raw;

      if (!value || /^https?:\/\//i.test(value)) {
        try {
          const parsed = new URL(value || String(pageUrl || ""));
    ...
    function defaultScopeForKind(kind) {
      return kind === "secret" ? "admin" : "ingest";
    }
    ...
    function normalizeScope(scopeRaw, kind) {
      ...
      for (const token of raw.split(",")) {
        ...
        if (!t) continue;
        if (t === "admin" || t === "dashboard" || t === "admin:all") hasAdmin = true;
        if (t === "ingest" || t === "events:write" || t === "analytics:write" || t === "analytics:events") {
          hasIngest = true;
        }
      }
    ...
    function normalizeAdminKeyHash(raw) {
      const hash = String(raw || "").trim().toLowerCase();
      return /^[a-f0-9]{64}$/.test(hash) ? hash : "";
    }

    function normalizeAdminKeyPrefix(raw, kindRaw) {
      const prefix = String(raw || "").trim().toLowerCase();
      if (prefix === "sk" || prefix === "secret") return "sk";
      if (prefix === "pk" || prefix === "publishable") return "pk";
      const kind = String(kindRaw || "").trim().toLowerCase();
      return kind === "secret" || kind === "admin" ? "sk" : "pk";
    }

    function normalizeWorkerScopeFromAdminInput(scopeRaw, keyPrefix) {
      const kind = keyPrefixToKind(keyPrefix);
      const tokens = Array.isArray(scopeRaw)
        ? scopeRaw
        : String(scopeRaw || "")
          .split(",")
          .map((s) => s.trim());
      ...
      if (kind === "secret" || hasAdmin) return "admin";
      if (hasIngest) return "ingest";
      return defaultScopeForKind(kind);
    }

    async function ensureProjectParentRow(projectId) {
      if (!ANALYTICS_DB) return;

      try {
        const accountInfo = await ANALYTICS_DB.prepare(`PRAGMA table_info(accounts)`).all();
        const accountColumns = Array.isArray(accountInfo?.results) ? accountInfo.results : [];
        if (accountColumns.length) {
          const accountColumnNames = new Set(accountColumns.map((col) => String(col.name || "").trim()).filter(Boolean));
          if (accountColumnNames.has("id")) {
            const existingAccount = await ANALYTICS_DB.prepare(`SELECT 1 AS ok FROM accounts WHERE id = ? LIMIT 1`)
              .bind(projectId)
              .first()
              .catch(() => null);
            if (!existingAccount?.ok) {
              const insertAccountColumns = ["id"];
              const accountValues = [projectId];
              const accountOptionalValues = {
                public_id: `account_${projectId}`,
              ...
              for (const col of accountColumns) {
                ...
                if (!accountOptionalValues[name]) continue;
                const notNull = Number(col.notnull || 0) === 1;
                const hasDefault = col.dflt_value != null;
                if (notNull && !hasDefault) {
                  insertAccountColumns.push(name);
                  accountValues.push(accountOptionalValues[name]);
                }
              }

              const placeholders = insertAccountColumns.map(() => "?").join(", ");
              const quotedColumns = insertAccountColumns.map((name) => `"${name}"`).join(", ");
              await ANALYTICS_DB.prepare(`INSERT OR IGNORE INTO accounts (${quotedColumns}) VALUES (${placeholders})`)
                .bind(...accountValues)
                .run()
                .catch(() => {});
            }
      ...
      let tableInfo;
      try {
        tableInfo = await ANALYTICS_DB.prepare(`PRAGMA table_info(projects)`).all();
      } catch {
        return;
      }
      ...
      const names = new Set(columns.map((col) => String(col.name || "").trim()).filter(Boolean));
      if (!names.has("id")) return;

      const existing = await ANALYTICS_DB.prepare(`SELECT 1 AS ok FROM projects WHERE id = ? LIMIT 1`)
        .bind(projectId)
        .first()
        .catch(() => null);
      ...
      for (const col of columns) {
        const name = String(col.name || "").trim();
        if (!name || name === "id") continue;
        if (!optionalValues[name]) continue;
        const notNull = Number(col.notnull || 0) === 1;
        const hasDefault = col.dflt_value != null;
        if (notNull && !hasDefault) {
          insertColumns.push(name);
          values.push(optionalValues[name]);
        }
      }

      const placeholders = insertColumns.map(() => "?").join(", ");
      const quotedColumns = insertColumns.map((name) => `"${name}"`).join(", ");
      await ANALYTICS_DB.prepare(`INSERT OR IGNORE INTO projects (${quotedColumns}) VALUES (${placeholders})`)
        .bind(...values)
        .run()
        .catch(() => {});
    }
    ...
    function resolveTrustedProxyProjectFromHeaders(rawKey) {
      const adminGate = requireAdminToken(request);
      if (!adminGate.ok) return null;

      const projectIdRaw = safeString(
        request.headers.get("X-Cavbot-Project-Id") ||
          request.headers.get("x-cavbot-project-id") ||
          "",
        40
      ).trim();
      if (!/^\d+$/.test(projectIdRaw)) return null;

      const projectId = Number(projectIdRaw);
      if (!Number.isInteger(projectId) || projectId <= 0) return null;

      const keyPrefix = normalizeKeyPrefixFromKey(rawKey);
      const kind = keyPrefixToKind(keyPrefix);
      return {
        projectId,
        kind,
        keyPrefix,
        source: "trusted_app_proxy",
        scope: normalizeScope("ingest", kind),
      };
    }
    ...
    // =========================
    async function resolveSite(projectId, pageUrl, reqOrigin, assertedOrigin, keyKind, sitePublicId) {
      ...
      const requestedPublicId = safeString(sitePublicId || "", 220).trim();
      ...
      // 0) Public site id match when the browser SDK sends it. Origin is still
      // validated below so a public id cannot move traffic across registered sites.
      if (requestedPublicId) {
        const byPublicId = await ANALYTICS_DB.prepare(
          `
          ...
          .bind(projectId, requestedPublicId)
          .first();

        if (byPublicId) {
          const registeredOrigin = String(byPublicId.origin || "");
          const compiled = compileOriginPattern(registeredOrigin);
          const originOk =
            !finalOrigin ||
            finalOrigin === registeredOrigin ||
            originMatchesPattern(finalOrigin, compiled);

          if (!originOk) {
            return {
              ok: false,
              error: "site_origin_mismatch",
              message: "Public site id does not match the request origin",
            };
          }
          ...
          return {
            ok: true,
            site: {
              id: Number(byPublicId.id),
              publicId: String(byPublicId.public_id),
              origin: registeredOrigin.includes("*") && finalOrigin ? finalOrigin : registeredOrigin,
              host: String(byPublicId.host || inferredHost || safeHostFromUrl(finalOrigin) || ""),
              label: String(byPublicId.label || byPublicId.host || registeredOrigin || finalOrigin),
            },
          };
        }
      }
    ...
    if (isAdminKeysPost) {
      ...
      const projectId = Number(adminKeysRouteMatch[1]);
      if (!Number.isInteger(projectId) || projectId <= 0) {
        return json(400, { ok: false, error: "invalid_project_id" }, corsHeaders);
      }

      const body = await readJsonSafe(request);
      const inputKeys = Array.isArray(body?.keys) ? body.keys : [];
      const statements = [];
      let synced = 0;
      let revoked = 0;
      let deleted = 0;
      ...
      for (const raw of inputKeys.slice(0, 100)) {
        const keyHash = normalizeAdminKeyHash(raw?.keyHash || raw?.key_hash || raw?.hash);
        if (!keyHash) {
          errors.push({ error: "invalid_key_hash" });
          continue;
        }

        if (raw?.delete === true || raw?.deleted === true) {
          statements.push(
            ANALYTICS_DB.prepare(`DELETE FROM project_keys WHERE key_hash = ?`).bind(keyHash)
          );
          deleted += 1;
          continue;
        }

        const keyPrefix = normalizeAdminKeyPrefix(raw?.keyPrefix || raw?.key_prefix || raw?.prefix, raw?.kind || raw?.type);
        const scope = normalizeWorkerScopeFromAdminInput(raw?.scope || raw?.scopes, keyPrefix);
        const revokedAtRaw = raw?.revokedAt || raw?.revoked_at || null;
        const revokedAt = revokedAtRaw ? safeString(revokedAtRaw, 80).trim() || new Date().toISOString() : null;
        ...
        statements.push(
          ...
          ).bind(projectId, keyPrefix, keyHash, scope, revokedAt)
        );
      }

      if (!statements.length) {
        return json(400, { ok: false, error: "no_valid_keys", errors }, corsHeaders);
      }

      await ensureProjectParentRow(projectId);
      await ANALYTICS_DB.batch(statements);
      return json(200, { ok: true, projectId, synced, revoked, deleted, errors }, corsHeaders);
    }
    ...
    if (isProjectSummary) {
      try {
        ...
        const PAGE_VIEW_SQL = "'cavbot_page_view','page_view','pageview','cavbot_pageview'";
        const ROUTE_CHANGE_SQL = "'cavbot_route_change','route_change','spa_navigation','navigation'";
        const JS_ERROR_SQL = "'cavbot_js_error','js_error','javascript_error','window_error','cavbot_error'";
        const API_ERROR_SQL = "'cavbot_api_error','api_error','fetch_error','http_error','network_error'";
        const NOT_FOUND_SQL = "'cavbot_404','404','not_found','page_404','cavbot_not_found'";

        const pct = (num, den) => (den > 0 ? Math.round((Number(num || 0) / den) * 1000) / 10 : null);
        const clampScore = (value) => Math.max(0, Math.min(100, Math.round(Number(value || 0))));
        const dbTimestampToIso = (value) => {
          const raw = safeString(value || "", 80).trim();
          if (!raw) return null;
        ...
        function buildSummaryPayload(input) {
          const totalEvents = Number(input.totalEvents || 0);
          const pageViews24h = Number(input.pageViews24h || 0);
          const pageViews = Number(input.pageViews || 0);
          const sessions = Number(input.sessions || 0);
          const visitors = Number(input.visitors || 0);
          const routesMonitored = Number(input.routesMonitored || 0);
          const views404 = Number(input.views404 || 0);
          const jsErrors = Number(input.jsErrors || 0);
          const apiErrors = Number(input.apiErrors || 0);
          const routeChanges = Number(input.routeChanges || 0);
          const topRoutes = Array.isArray(input.topRoutes) ? input.topRoutes : [];
          const trend = Array.isArray(input.trend) ? input.trend : [];
          const recentErrors = Array.isArray(input.recentErrors) ? input.recentErrors : [];
          const updatedAtISO = totalEvents > 0 ? safeString(input.updatedAtISO || new Date().toISOString(), 80) : null;
          const guardianScore =
            totalEvents > 0 ? clampScore(100 - jsErrors * 4 - apiErrors * 3 - views404 * 2) : null;

          const routeRows = topRoutes.map((r) => ({
            routePath: safeString(r.routePath || "/", 260) || "/",
            views: Number(r.views || 0),
            sessions: Number(r.sessions || 0),
            views404: Number(r.views404 || 0),
            jsErrors: Number(r.jsErrors || 0),
            apiErrors: Number(r.apiErrors || 0),
            lastSeenISO: r.lastSeenISO || null,
          }));

          return {
            project: { id: String(effectiveProjectId) },
            window: { range, days, label: `last_${days}d` },
            ...
            metrics: {
              ...
              uniqueVisitors30d: visitors,
              routesMonitored,
              views40430d: views404,
              sessions40430d: Number(input.sessions404 || 0),
              jsErrors30d: jsErrors,
              apiErrors30d: apiErrors,
              guardianScore,
              trend7d: trend.slice(-7),
              trend30d: trend.slice(-30),
              topRoutes: routeRows.slice(0, 12).map((r) => ({
            ...
            routes: {
              updatedAtISO,
              rollup: {
                routesObserved: routesMonitored,
                uniqueRoutes: routesMonitored,
                pageViews,
                sessions,
                routeChanges,
                views404Count: views404,
                jsErrorCount: jsErrors,
                views404Pct: pct(views404, pageViews),
                jsErrorPct: pct(jsErrors, pageViews),
              },
              topRoutes: routeRows,
            },
            seo: {
              updatedAtISO,
              rollup: {
                pagesObserved: routesMonitored,
                uniqueRoutes: routesMonitored,
                pageViews,
              },
              pages: routeRows.map((r) => ({
                urlPath: r.routePath,
                origin: siteOrigin || null,
                views: r.views,
                updatedAtISO: r.lastSeenISO || updatedAtISO,
              })),
            },
            errors: {
              updatedAtISO,
              totals: {
                ...
                apiErrors,
                views404,
                crashFreeSessionsPct: sessions > 0 ? pct(Math.max(0, sessions - Number(input.errorSessions || 0)), sessions) : null,
              },
              trend,
              groups: [],
              recent: recentErrors,
            ...
            controlRoom: {
              updatedAtISO,
              totals: {
                views404Total: views404,
                unique404Routes: routeRows.filter((r) => r.views404 > 0).length,
              },
              topRoutes: routeRows
                .filter((r) => r.views404 > 0)
                .map((r) => ({ routePath: r.routePath, views404: r.views404, source: "analytics" })),
              trend,
            },
          };
        }
        ...
        if (EVENTS_SCHEMA_V2) {
          ...
          const siteFilterSql = siteId ? "AND site_id = ?" : "";
          const bindSite = (values) => (siteId ? values.concat([siteId]) : values);

          const totalEvents = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
            ...
            .bind(...bindSite([effectiveProjectId, windowSql]))
            .first();
          ...
          const pv24 = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${PAGE_VIEW_SQL})
               AND created_at > datetime('now', '-1 day')
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId]))
            .first();

          const pageViews = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${PAGE_VIEW_SQL})
          ...
          const routeChanges = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${ROUTE_CHANGE_SQL})
               AND created_at > datetime('now', ?)
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
          ...
          const views404 = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${NOT_FOUND_SQL})
               AND created_at > datetime('now', ?)
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
          ...
          const sessions404 = await ANALYTICS_DB.prepare(
            `SELECT COUNT(DISTINCT session_key) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${NOT_FOUND_SQL})
          ...
          const jsErrors = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${JS_ERROR_SQL})
               AND created_at > datetime('now', ?)
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
          ...
          const apiErrors = await ANALYTICS_DB.prepare(
            `SELECT COUNT(*) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${API_ERROR_SQL})
               AND created_at > datetime('now', ?)
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
          ...
          const errorSessions = await ANALYTICS_DB.prepare(
            `SELECT COUNT(DISTINCT session_key) AS n
             FROM events
             WHERE project_id = ?
               AND event_name IN (${JS_ERROR_SQL},${API_ERROR_SQL},${NOT_FOUND_SQL})
               AND created_at > datetime('now', ?)
               ${siteFilterSql}`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
          ...
          const routeRows = await ANALYTICS_DB.prepare(
            `SELECT
               route_path AS routePath,
               SUM(CASE WHEN event_name IN (${PAGE_VIEW_SQL}) THEN 1 ELSE 0 END) AS views,
               COUNT(DISTINCT session_key) AS sessions,
               SUM(CASE WHEN event_name IN (${NOT_FOUND_SQL}) THEN 1 ELSE 0 END) AS views404,
               SUM(CASE WHEN event_name IN (${JS_ERROR_SQL}) THEN 1 ELSE 0 END) AS jsErrors,
               SUM(CASE WHEN event_name IN (${API_ERROR_SQL}) THEN 1 ELSE 0 END) AS apiErrors,
               MAX(event_timestamp) AS lastSeenISO
             FROM events
             WHERE project_id = ?
               AND created_at > datetime('now', ?)
               AND route_path IS NOT NULL
               ...
               ${siteFilterSql}
             GROUP BY route_path
             ORDER BY views DESC, lastSeenISO DESC
             LIMIT 100`
          )
            .bind(...bindSite([effectiveProjectId, windowSql]))
            .all();

          const trendRows = await ANALYTICS_DB.prepare(
            `SELECT
          ...
          const recentRows = await ANALYTICS_DB.prepare(
            `SELECT event_timestamp, event_name, route_path, page_url, payload_json
             FROM events
             WHERE project_id = ?
               AND event_name IN (${JS_ERROR_SQL},${API_ERROR_SQL},${NOT_FOUND_SQL})
               AND created_at > datetime('now', ?)
          ...
          const updated = await ANALYTICS_DB.prepare(
          ...
          return json(
            ...
            buildSummaryPayload({
              ...
              routeChanges: Number(routeChanges?.n || 0),
              views404: Number(views404?.n || 0),
              sessions404: Number(sessions404?.n || 0),
              jsErrors: Number(jsErrors?.n || 0),
              apiErrors: Number(apiErrors?.n || 0),
              errorSessions: Number(errorSessions?.n || 0),
              topRoutes: (routeRows?.results || []).map((r) => ({
                routePath: r.routePath,
                views: r.views,
                sessions: r.sessions,
                views404: r.views404,
              ...
              trend: (trendRows?.results || []).map((r) => ({
                day: safeString(r.day || "", 20),
                sessions: Number(r.sessions || 0),
                views404: Number(r.views404 || 0),
                jsErrors: Number(r.jsErrors || 0),
                apiErrors: Number(r.apiErrors || 0),
              })),
              recentErrors: (recentRows?.results || []).map((r) => ({
                tsISO: r.event_timestamp || null,
                kind: r.event_name || null,
                routePath: r.route_path || null,
                urlPath: r.page_url || null,
              })),
              updatedAtISO: dbTimestampToIso(updated?.ts),
            }),
            corsHeaders
          );
        }
        ...
        if (!rawKey) {
          return json(200, buildSummaryPayload({ schema: "legacy_empty", updatedAtISO: new Date().toISOString() }), corsHeaders);
        }
    ...
    if (isIngest) {
      try {
        ...
        let keyResolved = await resolveProjectFromKey(rawKey);
        if (!keyResolved) {
          keyResolved = resolveTrustedProxyProjectFromHeaders(rawKey);
        }
        if (!keyResolved) {
          await ensureProjectKeyRow(DEFAULT_PROJECT_ID, rawKey);
          keyResolved = await resolveProjectFromKey(rawKey);
        ...
        const siteRes = await resolveSite(
          keyResolved.projectId,
          pageUrl,
          origin,
          assertedSiteOrigin,
          keyResolved.kind,
          headerSitePublicId || bodySitePublicId
        );
        ...
        if (isV5) {
          ...
          for (let idx = 0; idx < body.records.length; idx++) {
            ...
            const name = normalizeEventName(r.event_name, eventType);
            ...
            const route_path = normalizeRoutePath(
              r?.route_path || payloadObj.routePath || payloadObj.path || payloadObj.urlPath || "",
              page_url
            );
        ...
        const base = {
          ...
          route_path: normalizeRoutePath(body.routePath || "", pageUrl),
        ...
        for (const e of events) {
          ...
          const name = normalizeEventName(e?.name, e?.type || payload?.event_type || payload?.type);
          if (!name) {
            dropped++;
            continue;
          }
          ...
          const routePath = normalizeRoutePath(
            e?.routePath || e?.path || payload?.routePath || payload?.path || payload?.urlPath || base.route_path,
            base.page_url
          );
          const mergedPayload = {
            ...payload,
