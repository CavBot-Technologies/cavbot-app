...
import { CONSOLE_ASSIST_REQUEST_SCHEMA } from "@/src/lib/ai/ai.types";
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (isStatusProbe) {
      return aiJson({ ok: true, requestId, probe: "ai_console_assist", accepted: true }, 200);
    }
    ...
    const parsed = CONSOLE_ASSIST_REQUEST_SCHEMA.safeParse(bodyRaw);
    if (!parsed.success) {
      return aiJson(
        {
          ok: false,
          requestId,
          error: "INVALID_INPUT",
          message: "Invalid Console assist payload.",
