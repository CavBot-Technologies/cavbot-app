...
import { getAccountActionCenterData, performAccountAction } from "@/lib/admin/operations.server";
...
function getRequiredScope(action: string) {
  if (action === "suspend" || action === "restore" || action === "revoke") return "security.write" as const;
  if (action === "extend_trial" || action === "change_plan" || action === "apply_credit" || action === "apply_comp") {
    return "plans.write" as const;
  }
  return "accounts.write" as const;
}

export async function GET(req: NextRequest, { params }: { params: { accountId: string } }) {
  try {
    await requireAdminAccess(req, { scopes: ["accounts.read"] });
    const data = await getAccountActionCenterData(params.accountId);
    if (!data) return adminJson({ ok: false, error: "ACCOUNT_NOT_FOUND" }, 404);
    return adminJson({ ok: true, data });
  } catch (error) {
    if (error instanceof ApiAuthError) {
      return adminJson({ ok: false, error: error.code }, error.status);
    }
    return adminJson({ ok: false, error: "ACCOUNT_ACTIONS_READ_FAILED" }, 500);
  }
}
...
export async function POST(req: NextRequest, { params }: { params: { accountId: string } }) {
  try {
    ...
    if (!action) return adminJson({ ok: false, error: "ACCOUNT_ACTION_REQUIRED" }, 400);
    ...
    const result = await performAccountAction({
      actor: {
    ...
    return adminJson({
      ok: false,
      error: error instanceof Error ? error.message : "ACCOUNT_ACTION_FAILED",
    }, 500);
  }
}
