...
import { listCavPadDirectoryShares, shareCavPadDirectoryByIdentity } from "@/lib/cavpad/server";
...
export async function GET(req: Request, ctx: { params: { id?: string } }) {
  try {
    ...
    const accessList = await listCavPadDirectoryShares({
    ...
    return cavcloudErrorResponse(err, "Failed to load CavPad folder shares.");
  }
}
...
export async function POST(req: Request, ctx: { params: { id?: string } }) {
  try {
    ...
    const shared = await shareCavPadDirectoryByIdentity({
      accountId: String(sess.accountId || ""),
      userId: String(sess.sub || ""),
      directoryId,
      identity,
    ...
    return cavcloudErrorResponse(err, "Failed to share CavPad folder.");
  }
}
