...
import { revokeCavPadNoteShare, updateCavPadNoteShare } from "@/lib/cavpad/server";
...
export async function PATCH(req: Request, ctx: { params: { id?: string; shareId?: string } }) {
  try {
    ...
    if (!noteId || !shareId) {
      return jsonNoStore({ ok: false, error: "BAD_REQUEST", message: "note id and share id are required." }, 400);
    }
    ...
    const updated = await updateCavPadNoteShare({
    ...
    return cavcloudErrorResponse(err, "Failed to update CavPad note share.");
  }
}
...
export async function DELETE(req: Request, ctx: { params: { id?: string; shareId?: string } }) {
  try {
    ...
    const removed = await revokeCavPadNoteShare({
      accountId: String(sess.accountId || ""),
      userId: String(sess.sub || ""),
      noteId,
      shareId,
    ...
    return cavcloudErrorResponse(err, "Failed to revoke CavPad note share.");
  }
}
