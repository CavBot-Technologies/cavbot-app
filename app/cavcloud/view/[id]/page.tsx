...
function previewRawSrc(source: CavCloudPreviewSource, resourceId: string, byPath: string): string {
  if (source === "artifact") {
    return `/api/cavcloud/artifacts/${encodeURIComponent(resourceId)}/preview?raw=1`;
  }
  if (source === "trash") {
    return `/api/cavcloud/trash/${encodeURIComponent(resourceId)}?raw=1`;
  }
  if (source === "by_path") {
    return `/api/cavcloud/files/by-path?path=${encodeURIComponent(byPath)}&raw=1&access=1`;
  }
  return `/api/cavcloud/files/${encodeURIComponent(resourceId)}?raw=1&access=1`;
}
...
function buildViewerHref(args: {
  ...
  return `/cavcloud/view/${encodeURIComponent(args.resourceId)}?${params.toString()}`;
}

export default function CavCloudFileViewerPage() {
  ...
  const uploadedBy = uploadedByParam || resolvedUploadedBy || "CavCloud user";
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/cavcloud/files/${encodeURIComponent(resourceId)}`, {
          method: "GET",
          cache: "no-store",
          signal: ctrl.signal,
        });
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setResolvedUploadedBy((prev) => prev || "CavCloud user");
      }
    })();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/cavcloud/tree?folder=${encodeURIComponent(activeFolderPath)}`, {
          method: "GET",
          cache: "no-store",
          signal: ctrl.signal,
        });
        ...
        {item ? (
          <CavCloudPreviewPanel
            ...
            onClose={() => {
              ...
              router.push(`/cavcloud?folderPath=${folderPath}`);
            }}
            onOpen={() => {}}
            onCopyLink={copyLink}
            onShare={() => {}}
          ...
          <div className="cc-viewerEmpty">Preview unavailable. Use Open from CavCloud and try again.</div>
        )}
      </div>
      {toasts.length ? (
