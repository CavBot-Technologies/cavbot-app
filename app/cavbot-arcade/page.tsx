...
const HERO_VIDEO_SRC = "/cavbot-arcade/entertainment/main-preview.mp4";
...
const ARCADE_LIBRARY_GAMES = GAMES.slice(0, 3);
...
const ArcadePage = () => {
  ...
  const [heroVideoArmed, setHeroVideoArmed] = useState(false);
  ...
  const armHeroVideo = () => {
    ...
    setHeroVideoArmed(true);
  };

  const disarmHeroVideo = () => {
    setHeroVideoArmed(false);
  };
  ...
  useEffect(() => {
    if (!heroVideoArmed) return undefined;
    const video = heroVideoRef.current;
    if (!video) return undefined;
    video.currentTime = 0;
    const playPromise = video.play();
  ...
  const shouldShowHeroVideo = heroVideoReady && heroVideoArmed;
        ...
        <main className="arcade-main">
          ...
          <div className="ambient">
            {[
		          ...
		          <section className="hero">
		            ...
		            <div className="hero-card" aria-label="CavBot Arcade preview">
		              <div className="hero-card-shell">
		                ...
		                <div
                      className={`hero-card-screen ${shouldShowHeroVideo ? "is-video-ready" : ""}`}
                      onMouseEnter={armHeroVideo}
                      onMouseLeave={disarmHeroVideo}
                      onPointerEnter={armHeroVideo}
                      onPointerLeave={disarmHeroVideo}
                      onTouchStart={armHeroVideo}
                      onFocus={armHeroVideo}
                      onBlur={disarmHeroVideo}
                      tabIndex={0}
                    >
		                  <Image
		                    src="/cavbot-arcade/cavbot-arcade-thumbnail.png"
		                    ...
		                    priority
		                  />
                      <video ref={heroVideoRef} muted playsInline loop preload="none" />
		                </div>
		              </div>
		            </div>
		          </section>
              ...
              <div className="gallery-header-featured">
                <Link
                  href="/cavbot-arcade/gallery"
                  className="gallery-featured-link"
                  aria-label="View all CavBot arcade games"
          ...
          <section className="devices">
            ...
            <div className="device-grid">
              {[
                {
                  label: "Phone",
                  copy: "Designed for touch. Fast, tap-friendly rounds for on-the-go play. ",
                  iconSrc: "/icons/app/cavcode-viewer/phone-svgrepo-com.svg",
                },
                {
                  label: "Tablet",
                  copy: "A delightful Arcade experience with enhanced control and clarity.",
                  iconSrc: "/icons/app/cavcode-viewer/tablet-svgrepo-com.svg",
                },
                {
                  label: "Desktop",
                  copy: "Full Arcade layout with precision controls and immersive previews..",
                  iconSrc: "/icons/app/cavcode-viewer/desktop-svgrepo-com.svg",
                },
              ].map((device) => (
          ...
          <section className="faq">
            ...
            <div className="faq-list">
              {[{
                question: "What is the CavBot Arcade?",
                answer: `CavBot Arcade exists to reframe failure as opportunity.
A broken route isn’t an error to hide—it’s a moment to re-engage. By transforming 404s into calm, intentional interactive experiences, CavBot Arcade keeps visitors present, restores momentum, and turns dead ends into meaningful signal.`,
              }, {
                question: "Why games on a 404 product?",
                answer: `Because a 404 is a moment of attention. Someone arrived with intent—and the path disappeared.
Games turn that pause into engagement, restore momentum, and give visitors a reason to continue instead of leave.
A 404 doesn’t need an apology. It needs a better experience.`,
              }, {
                question: "Are these games playable on mobile?",
                answer: `Yes.
Every game is built to work seamlessly across phone, tablet, and desktop—adapting naturally to each screen without compromise.`,
              }, {
                question: "Does sound matter?",
                answer: `Sound is optional.
All games are fully playable without audio, with sound designed as a subtle enhancement—not a requirement.`,
              }].map((item) => (
