...
- AppShell plan hydration could miss the cached shell snapshot before auth bootstrap settled on the current `origin/main`.
- CavAi pack retrieval on the current `origin/main` needed explicit account/session context in the route source.
...
- App shell and CavAi pack routing: `components/AppShell.tsx`, `app/api/cavai/packs/route.ts`.
- Billing settings lint/type cleanup: `app/settings/sections/BillingClient.tsx`.
...
- Preserved cached AppShell plan state before auth bootstrap to reduce Premium/free lock flashes.
- Kept CavAi pack retrieval explicitly authenticated, account-scoped, and origin-scoped.
...
- `npm run test:sdk` passed: 62/62.
- `node --import tsx --test tests/cavai-domain-middleware.test.ts tests/cavai-domain-url.test.ts tests/project-summary-placeholder-vitals.test.ts tests/billing-plan-source-of-truth.test.ts` passed: 16/16.
- `npm run lint` passed with 0 errors and 28 pre-existing warnings in unrelated/admin/large app areas.
